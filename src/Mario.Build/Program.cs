...
Target("compile", DependsOn("clean"), () =>
    Run("dotnet",  $"build src/Mario.sln --framework {framework} --configuration {configuration}"));

Target("test-sample-api", DependsOn("compile"), () =>
    RunTests("Mario.Sample.Api.Tests"));

Target("test", DependsOn("test-sample-api"));

Target("pack", DependsOn("compile"), ForEach("./src/Mario"), project =>
